"""Integration tests for Panoptic — lifecycle, project management, users, plugins."""
import shutil
...
@pytest.fixture
def panoptic(tmp_path: Path) -> Generator[Panoptic, Any, None]:
...
def test_create_project(panoptic: Panoptic, project_folder: Path):
...
def test_create_project_duplicate_path_raises(panoptic: Panoptic, project_folder: Path):
    panoptic.create_project('First', project_folder)
    with pytest.raises(ValueError, match='already registered'):
        panoptic.create_project('Second', project_folder)


def test_get_projects_returns_created(panoptic: Panoptic, project_folder: Path):
...
def test_import_project(panoptic: Panoptic, tmp_path: Path):
    # Seed a project folder using Project directly
    folder = tmp_path / 'existing_project'
    with Project(folder) as proj:
        uid = proj.config.id
...
def test_import_project_no_db_raises(panoptic: Panoptic, tmp_path: Path):
...
def test_import_project_duplicate_path_raises(panoptic: Panoptic, tmp_path: Path):
...
def test_load_project_returns_project2(panoptic: Panoptic, project_folder: Path):
    key = panoptic.create_project('P', project_folder)
...
def test_load_project_same_instance_returned_twice(panoptic: Panoptic, project_folder: Path):
    key = panoptic.create_project('P', project_folder)
...
def test_load_project_unknown_uid_raises(panoptic: Panoptic):
    with pytest.raises(ValueError, match='not registered'):
        panoptic.load_project('nonexistent-id')


def test_get_project_before_load_returns_none(panoptic: Panoptic, project_folder: Path):
    key = panoptic.create_project('P', project_folder)
    assert panoptic.get_project(key.id) is None


def test_get_project_after_load_returns_instance(panoptic: Panoptic, project_folder: Path):
    key = panoptic.create_project('P', project_folder)
    panoptic.load_project(key.id)
    assert panoptic.get_project(key.id) is not None


def test_close_project_removes_from_loaded(panoptic: Panoptic, project_folder: Path):
    key = panoptic.create_project('P', project_folder)
    panoptic.load_project(key.id)
    assert panoptic.get_project(key.id) is not None
...
def test_close_project_not_loaded_is_noop(panoptic: Panoptic, project_folder: Path):
    key = panoptic.create_project('P', project_folder)
    panoptic.close_project(key.id)  # should not raise


def test_delete_project_removes_registration(panoptic: Panoptic, project_folder: Path):
    key = panoptic.create_project('P', project_folder)
    panoptic.delete_project(key.id)
    assert panoptic.get_projects() == []


def test_delete_project_with_files_removes_folder(panoptic: Panoptic, project_folder: Path):
    key = panoptic.create_project('P', project_folder)
...
def test_delete_project_without_files_keeps_folder(panoptic: Panoptic, project_folder: Path):
    key = panoptic.create_project('P', project_folder)
    panoptic.delete_project(key.id, delete_files=False)
    assert project_folder.exists()


def test_update_project_name(panoptic: Panoptic, project_folder: Path):
...
def test_update_project_excluded_plugins(panoptic: Panoptic, project_folder: Path):
    key = panoptic.create_project('P', project_folder)
    updated = panoptic.update_project(key.id, excluded_plugins=['plugin_a'])
    assert updated.excluded_plugins == ['plugin_a']


def test_load_project_uuid_mismatch_raises(panoptic: Panoptic, tmp_path: Path):
...
def test_create_user(panoptic: Panoptic):
...
def test_create_user_duplicate_name_raises(panoptic: Panoptic):
    panoptic.create_user('alice')
    with pytest.raises(ValueError, match='already exists'):
        panoptic.create_user('alice')


def test_get_user_by_name(panoptic: Panoptic):
    panoptic.create_user('alice')
...
def test_get_user_by_name_missing_returns_none(panoptic: Panoptic):
    assert panoptic.get_user_by_name('nobody') is None


def test_delete_user(panoptic: Panoptic):
...
def test_login_returns_token(panoptic: Panoptic):
    panoptic.create_user('alice', password='secret')
    token = panoptic.login('alice', 'secret')
    assert isinstance(token, str) and len(token) > 0


def test_login_wrong_password_raises(panoptic: Panoptic):
    panoptic.create_user('alice', password='secret')
    with pytest.raises(ValueError, match='Invalid credentials'):
        panoptic.login('alice', 'wrong')


def test_login_unknown_user_raises(panoptic: Panoptic):
    with pytest.raises(ValueError, match='Invalid credentials'):
        panoptic.login('nobody', 'pw')


def test_get_user_from_token(panoptic: Panoptic):
...
def test_get_user_from_invalid_token_returns_none(panoptic: Panoptic):
    assert panoptic.get_user_from_token('bad-token') is None


def test_logout_invalidates_token(panoptic: Panoptic):
    panoptic.create_user('alice', password='pw')
    token = panoptic.login('alice', 'pw')
...
def test_each_login_returns_distinct_token(panoptic: Panoptic):
    panoptic.create_user('alice', password='pw')
...
def test_get_state(panoptic: Panoptic, project_folder: Path):
    key = panoptic.create_project('P', project_folder)
    panoptic.load_project(key.id)
