...
from panoptic.core.panoptic.models import ProjectState
...
class Panoptic:
    ...
    def create_project(self, name: str, path: str | Path) -> ProjectKey:
        ...
        # Project.start() creates the folder and seeds the DBs
        project = Project(p)
        project.start()
    ...
    def load_project(self, id_: str) -> Project:
        """Start a Project instance for the given id and keep it in memory."""
        with self._lock:
        ...
        project = Project(Path(key.path), plugin_keys=project_plugins)
        project.start()
    ...
    def get_project(self, id_: str) -> Optional[Project]:
        with self._lock:
