...
<template>
    <template v-if="loaded" class="container">
        <div v-if="item.type == 'group'">
            <GroupLineVue :prop-values="item.data.propertyValues" :item="(item as GroupLine)" :width="props.width" :data="props.data"
                @close:group="e => emits('close:group', e)" @open:group="e => emits('open:group', e)" @toggle:group="e => emits('toggle:group', e)"/>
        </div>
        <div v-if="item.type == 'image'">
            <RowLine :item="item" :properties="props.properties" :show-image="props.showImages"
                ...
                @toggle:image="e => emits('toggle:image', e)"
                :selected="props.selectedImages[item.data.id]" />
        </div>
        <div v-if="item.type == 'pile'">
            <RowLine :item="item" :properties="props.properties" :show-image="props.showImages"
                :missing-width="props.missingWidth" @resizeHeight="h => emits('resizeHeight', h)"
                :selected="props.selectedImages[item.data.images[0].id]" @toggle:image="e => emits('toggle:image', e)" />
        </div>
